...
spike_out = None
rollback_events = None    # [research] rollback-triggering belated spike log (Stage1 #3 / Stage2 distance)
redo_cyc = None           # [research Stage3-B] per-core cycles spent in rollback_state (= wasted speculation mass)
redo_restore_cyc = None   # [research Stage4-energy] redo_cyc 분해: 복원/부기 오버헤드(belated 1건당 고정 2cyc)
redo_recompute_cyc = None # [research Stage4-energy] redo_cyc 분해: state_update 재계산/복구 replay (delta_t 비례). restore+recompute == redo_cyc 불변
fsm_cyc = None            # [research Stage4-energy] per-core FSM 상태별 사이클 분해 [6]: 0=neu_comp(유용연산) 1=neu_comp_end 2=sync_wait(idle) 3=commit 4=sync_done 5=rollback(==redo_cyc). Σ상태 == cyc 불변
