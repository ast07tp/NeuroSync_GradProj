...
cdef class Core:
    ...
    cpdef core_advance(self):
        ...
        cdef int entry_state    # [research Stage4-energy] 이 틱이 처리하는 FSM 상태(분해 집계용)

        entry_state = <int>self.FSM_state
        ...
        # Rollback Stage
            ...
            # [research Stage3-B] 이 분기 진입 = 1 HW cycle 을 rollback_state 에서
            # 소모(아래 line 242 GV.cyc += 1 이 step 마다 무조건 실행). 순수 관찰:
            # FSM·rollback_consuming_cyc·spike 출력 불변 → 낭비투기 사이클 정확 집계.
            GV.redo_cyc[self.ind] += 1
        ...
        GV.cyc[self.ind] += 1
        # [research Stage4-energy] 이 틱을 진입 상태에 귀속(순수 관찰). Σ fsm_cyc == cyc,
        # 그리고 fsm_cyc[rollback_state] == redo_cyc 가 자체 검증으로 성립해야 함.
        GV.fsm_cyc[self.ind][entry_state] += 1
