...
cdef class RRManager:
    ...
    cpdef int rollback(self):
        ...
        cdef int _setup_consuming   # [research Stage4-energy] redo_cyc 분해용 스냅샷
        ...
        # [research] log the rollback-triggering belated spike
        #   spiked_timestep   : 트리거 belated spike 가 발화된 timestep (Stage1 #3 의 x축)
        #   affecting_timestep: 그 spike 가 영향을 줬어야 하는 timestep
        #   delta_t           : 롤백 거리 (Stage2 롤백강도용)
        GV.rollback_events.append((spiked_timestep, affecting_timestep, delta_t, src_pid, int(is_anti), rollback_gid))
        ...
        # [research Stage4-energy] 여기까지 = 복원/부기(체크포인트 retrieve). 이후 루프는
        # 전부 state_update 재계산(recompute/recovery). 스냅샷으로 둘을 분해.
        _setup_consuming = self.rollback_consuming_cyc
        ...
        # [research Stage4-energy] redo_cyc 분해 집계 (순수 관찰; consuming_cyc 만 읽음).
        # restore = 체크포인트 retrieve 고정비, recompute = 이후 state_update replay 전부
        # (cascading wrong_behavior_recovery 포함). 합 == 이 이벤트의 rollback_consuming_cyc.
        GV.redo_restore_cyc[self.ind] += _setup_consuming
        GV.redo_recompute_cyc[self.ind] += self.rollback_consuming_cyc - _setup_consuming
